...
def create_train_val_test_splits(real_count, fake_count, train_ratio=0.7, val_ratio=0.15):
    ...
    for ext in ['.jpeg', '.png']:
        real_images.extend((processed_dir / "real").glob(f"*{ext}"))
        fake_images.extend((processed_dir / "fake").glob(f"*{ext}"))
...
def create_summary_report():
    ...
    for split in ['train', 'val', 'test']:
        ...
        if split_path.exists():
            ...
            fake_count = len(list((split_path / 'fake').glob('*.*')))
